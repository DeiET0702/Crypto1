...
class Keychain {
    ...
    static new(keychainPassword) {
        ...
        keychain.secrets.salt = salt; // Lưu salt ban đầu

        console.log("New Keychain created with salt:", Keychain.encodeBuffer(salt)); // Debug
        return keychain;
    }

    // Load a keychain from a serialized representation
    static load(keychainPassword, repr, trustedDataCheck = null) {
        const keychain = new Keychain();
        const data = JSON.parse(repr);

        if (trustedDataCheck) {
            const checksum = crypto.createHash('sha256').update(repr).digest();
            if (!checksum.equals(trustedDataCheck)) {
        ...
        const masterKey = Keychain.generateKey(keychainPassword, salt, PBKDF2_ITERATIONS, AES_KEY_LENGTH);
        const hmacKey = Keychain.decodeBuffer(data.hmacKey); // Khôi phục hmacKey từ dữ liệu đã lưu

        keychain.secrets.masterKey = masterKey;
        keychain.secrets.hmacKey = hmacKey;
        keychain.secrets.salt = salt; // Lưu salt từ dữ liệu đã lưu
        keychain.data.kvs = data.kvs;

        console.log("Keychain loaded with salt:", Keychain.encodeBuffer(salt)); // Debug
        return keychain;
    }
    ...
    dump() {
        const repr = JSON.stringify({
            kvs: this.data.kvs,
            salt: Keychain.encodeBuffer(this.secrets.salt), // Sử dụng salt đã lưu
            hmacKey: Keychain.encodeBuffer(this.secrets.hmacKey), // Lưu hmacKey
        });
        const checksum = crypto.createHash('sha256').update(repr).digest();
    ...
    get(domain) {
        ...
        return Keychain.decryptAESGCM(this.secrets.masterKey, Keychain.decodeBuffer(iv), Keychain.decodeBuffer(encrypted), Keychain.decodeBuffer(tag));
    }
